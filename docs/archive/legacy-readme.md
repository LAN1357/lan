...
> 以下内容保存历史设计，不代表当前产品能力、隐私边界或财务口径。当前说明以仓库根目录 README.md 为准。

# 直播经营复盘工作台

当前浏览器工作台提供三个一级入口：月份总览、M2结算工作台和M3只读经营分析。流程为标准 Excel 导入 → 固定规则生成候选 → 人工批量确认 → 费用维护与已确认方案复用 → 双利润 → 月度关账 → Excel / 浏览器分析。M1 核算口径保持稳定。

月中可暂不录入月度费用，先在M2预览经营复盘利润；预览仍要求收入与直接成本齐备，不形成正式关账版本。月末确认完整费用后再关账。M3页首新增“使用 Claude Code / Codex 分析本月”，可复制带版本和筛选范围的分析指令，并查看客户端连接检查说明。
...
打开 http://127.0.0.1:8765 。浏览器看板不接入模型；自然语言对话继续通过Codex或Claude Code中的只读MCP完成。使用独立 `data/workbench.db`。首次打开 M1 工作台库时自动备份并升级到结构版本2；旧关账副本保留。

- [工作台使用说明与两场演练](docs/workbench-quickstart.md)
- [标准模板](docs/templates/经营复盘标准模板.xlsx)
- [合成两场账例](docs/templates/经营复盘合成两场账例.xlsx)
- [精简 M1 实施记录](docs/superpowers/plans/2026-09-05-m1-minimal-workbench.md)
- [M2 功能、验证结果与业务验收清单](docs/superpowers/plans/2026-09-05-m2-bp-workflow.md)

macOS桌面开发验证版已增加pywebview外壳，仍复用同一个本地HTTP工作台和外部SQLite账本。开发启动、账本位置、进程锁及打包方式见[桌面版使用说明](docs/desktop-quickstart.md)。未完成财务BP完整操作、文件对话框和干净机器验收前，浏览器入口继续保留为稳定回退。

M3 已建立浏览器与MCP共用的只读分析服务。它只读取 `months` 和 `closes` 中的不可变关账副本，提供八个经营查询、证据、固定规则诊断和非持久化条件测算能力；不会导入、改数、确认或关账。对话中的Agent有分析主动权，可在用户明确范围内自主规划、连续查询、追查证据、提出待验证假设并比较方案；正式业务操作仍由工作台和BP完成。浏览器分析直接随上面的工作台服务启动；下面的stdio命令仅用于MCP客户端管理，不要当作交互式终端使用。
...
- [M3 接入与使用说明](docs/m3-readonly-analysis.md)
- [M3 实施与验证记录](docs/superpowers/plans/2026-09-06-m3-readonly-analysis.md)
- [M3 浏览器经营分析看板整合计划](docs/superpowers/plans/2026-09-06-m3-browser-dashboard-integration.md)
- [pywebview桌面版设计与实施状态](docs/superpowers/specs/2026-09-07-pywebview-desktop-design.md)

当前支持标准模板，尚未经过脱敏真实平台导出试用。M3目前完成合成账例与本地现有库验证，尚未经过财务BP真实问题集验收。下面保留的旧 MCP 原型文档不是新版已实现能力或核算口径的依据。
...
这里的旧 MCP 原型说明仅用于历史参考；旧 `roi-analyst` 角色已归档，不作为当前工作台或 M3 分析入口。当前 M3 的浏览器与只读 MCP 使用方式请参阅 `docs/m3-readonly-analysis.md`。

以下步骤同样仅描述旧原型流程，不应在当前工作台中执行：
